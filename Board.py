...
class Board:
  ...
  def collectNeighbors( self, i, j ):
    ...
    for dr, dc in directions:
      ...
      if 0 <= r < rows and 0 <= c < cols:
        ...
        if self.m_Patches[r][c]:
          neighbors.append(self.m_Mines[r][c])
        else:
          neighbors.append(9)
        # end if
      else:
    ...
    # Save neighbors 
    with open("X.csv", mode="a", newline="") as fx:
        writer = csv.writer(fx)
        writer.writerow(neighbors)

    # Save result
    result = 1 if self.m_Mines[i][j] == 9 else 0
    with open("y.csv", mode="a", newline="") as fy:
